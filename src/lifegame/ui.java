...
public class ui extends JFrame implements ActionListener {
    ...
    public void initGUI() {
        ...
        Log = new logic(maxWidth, maxLength);
        ...
        jbNowGeneration = new JButton(""+Log.getNowGeneration());//Buttom不能直接添加int，故采用此方式
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == ok) { //确定
            ...
            Log = new logic(getMaxWidth(), getMaxLength());

        } else if(e.getSource() == clearGeneration){ //代数清零
            Log.setNowGeneration(0);
            jbNowGeneration.setText(""+Log.getNowGeneration());//刷新当前代数
            isRunning = false;
            thread = null;
